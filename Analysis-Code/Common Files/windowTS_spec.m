function [x_mat] = windowTS_spec(x,winSize,winStep,pad,winType)
...
ncol = fix((length(x)-(winSize-winStep))/(length(winVect)-(winSize-winStep)));

% Pre-process X
colindex = 1 + (0:(ncol-1))*(length(winVect)-(winSize-winStep));
rowindex = (1:length(winVect))';
x_mat = zeros(length(winVect),ncol);

% Put x into columns of xin with the proper offset
x_mat(:) = x(rowindex(:,ones(1,ncol))+colindex(ones(length(winVect),1),:)-1);

% Apply the window to the array of offset signal segments.
x_mat = winVect(:,ones(1,ncol)).*x_mat;
