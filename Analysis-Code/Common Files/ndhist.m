...
if isreal(z)
    if size(z,2)==2 % we have a [x y] case
        [edgesX2,edgesY2,N, h] = ndhist(z(:,1)+1i*z(:,2),varargin{:});
        ...
        if isempty(varargin) % just one value passed
            ...
            idx = (1:length(z))';
            [edgesX2,edgesY2,N, h] = ndhist(idx+1i*z(:),'fx',4,'fy',2);
            ...
            if isnumeric(y) % we've got the (x,y) case
                ...
                [edgesX2,edgesY2,N, h] = ndhist(z(:)+1i*y(:),varargin{2:end});
            else % we've got just one value, but with special arguments passed
%                 display('your data are all ''real'' so we intepreted it as a timeseries');
                idx = (1:length(z))';
                [edgesX2,edgesY2,N, h] = ndhist(idx+1i*z(:),'fx',4,'fy',1,varargin{:});
            end
        end
    end
    if nargout==1
...
%             Should I add all those puppies in?
        ...
        case {'binsy','fy'} % 'f' comes from the binfactor of nhist
            binFactorY=varargin{k+1};
            k = k + 1;
        case {'edgesx'} % xbins vector passed by the user
            userEdgesFlagX=1;
            userEdgesX=varargin{k+1};
            k = k + 1;
         case {'edgesy'} % ybins vector passed by the user
            userEdgesFlagY=1;
            userEdgesY=varargin{k+1};
            k = k + 1;   
...
% Normalize the counts matrix by total counts so sum(P(x))==1
if probFlag
%     N=N/(binWidthX*binWidthY*length(x));
    N=N/(sum(N(:)));
end
