function [DTWdist p] = DTWsubWeightedNormBand(F1,F2)
...
%
% This is a classical DTW. No extra conditions are assigned. Computes
% optimal path through the distance matrix.2
%
...
for i = 1:min(M1,Band+1)
    ...
    for j = 1:i
        D(i,1) = D(i,1)+norm(F1(:,j)-F2(:,1))^2/norm(F1(:,j))/norm(F2(:,1));%cost(F1,F2,j,1);
    end
end


for i = 1:min(M2,Band+1)   
    D(1,i) = norm(F1(:,1)-F2(:,i))^2/norm(F1(:,1))/norm(F2(:,i));
end

for i = 2:min(M1,Band+2)
        dij = norm(F1(:,i)-F2(:,2))^2/norm(F1(:,i))/norm(F2(:,2)); 
        D(i,2) = min(D(i-1,1)+dij,min(D(i-1,2)+dij/2,D(i,1)+dij/2));
end

for j = 2:min(M2,Band+2)
        dij = norm(F1(:,2)-F2(:,j))^2/norm(F1(:,2))/norm(F2(:,j));
...
for j = 3:M2
    for i = max(3,j-Band):min(M1,j+Band)
        ...
        dij = norm(F1(:,i)-F2(:,j))^2/norm(F1(:,i))/norm(F2(:,j));
        dim1j = norm(F1(:,i-1)-F2(:,j))^2/norm(F1(:,i-1))/norm(F2(:,j));
        dim1jm1=norm(F1(:,i-1)-F2(:,j-1))^2/norm(F1(:,i-1))/norm(F2(:,j-1));
        dijm1 = norm(F1(:,i)-F2(:,j-1))^2/norm(F1(:,i))/norm(F2(:,j-1));
