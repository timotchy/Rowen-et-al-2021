...
function [DTWdist p] = DTWsubBand(F1,F2)


[L1 M1] = size(F1);
[L2 M2] = size(F2);

%Band = 50;  % maximum distance (in time bins) for start and end of song from end of file/data
Band = 50;  % maximum distance (in time bins) for start and end of song from end of file/data
...
for j = 2:M2
    for i = max(2,j-Band):min(M1,j+Band)
