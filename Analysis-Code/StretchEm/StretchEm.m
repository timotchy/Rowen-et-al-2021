...
if isequal(file,0) || isequal(path,0)
    ...
    if ~isempty(handles.annotationFilename)
        ...
        if (strcmp(button,'Replace current'))
            ...
            %Only continue with DAT and STM files
            idxD = contains(handles.keys, '.dat');
            idxS = contains(handles.keys, '.stm');
            handles.keys = handles.keys(idxD | idxS);
            handles.elements = handles.elements(idxD | idxS);
