function varargout = TweetVision_PC(varargin)
% TweetVision_PC M-file for TweetVision_PC.fig
%      TweetVision_PC, by itself, creates a new TweetVision_PC or raises the existing
%      singleton*.
%
%      H = TweetVision_PC returns the handle to a new TweetVision_PC or the handle to
%      the existing singleton*.
%
%      TweetVision_PC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TweetVision_PC.M with the given input arguments.
%
%      TweetVision_PC('Property','Value',...) creates a new TweetVision_PC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TweetVision_PC_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TweetVision_PC_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TweetVision_PC
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @TweetVision_PC_OpeningFcn, ...
                   'gui_OutputFcn',  @TweetVision_PC_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before TweetVision_PC is made visible.
function TweetVision_PC_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TweetVision_PC (see VARARGIN)

% Choose default command line output for TweetVision_PC
...
%%% This part is for the .DAT functionality of TweetVision_PC
...
% UIWAIT makes TweetVision_PC wait for user response (see UIRESUME)
% uiwait(handles.TweetVision_PC);
setappdata(0  , 'hTweetVision_PC'    , gcf);
...
function varargout = TweetVision_PC_OutputFcn(hObject, eventdata, handles) 
...
hTweetVision_PC = getappdata(0, 'hTweetVision_PC');
handles = guidata(hTweetVision_PC);
fs = handles.exper.desiredInSampRate;
filenum = handles.filenum;
mouseMode = get(get(hObject,'Parent'), 'SelectionType');
...
guidata(hTweetVision_PC, handles);
...
% Get workspace name
hTweetVision_PC = getappdata(0, 'hTweetVision_PC');
handles.PreProcess = getappdata(hTweetVision_PC, 'PreProcess');
handles.plotdata = [];
minval = 0;
maxval = -10;
...
%directory_name = 'C:\Users\Tim\Desktop\MATLAB Scripts\Tim''s Scripts\TweetVision_PC';
...
% Get workspace name
hTweetVision_PC = getappdata(0, 'hTweetVision_PC');
setappdata(hTweetVision_PC, 'PreProcess', handles.PreProcess);

TweetVisProcOptions(handles.PreProcess,1,'hTweetVision_PC');
