function varargout = FinchShocker_v2(varargin)
% FINCHSHOCKER_V2 MATLAB code for FinchShocker_v2.fig
%      FINCHSHOCKER_V2, by itself, creates a new FINCHSHOCKER_V2 or raises the existing
%      singleton*.
%
%      H = FINCHSHOCKER_V2 returns the handle to a new FINCHSHOCKER_V2 or the handle to
%      the existing singleton*.
%
%      FINCHSHOCKER_V2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FINCHSHOCKER_V2.M with the given input arguments.
%
%      FINCHSHOCKER_V2('Property','Value',...) creates a new FINCHSHOCKER_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FinchShocker_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FinchShocker_v2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FinchShocker_v2

% Last Modified by GUIDE v2.5 19-Apr-2018 09:28:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FinchShocker_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @FinchShocker_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FinchShocker_v2 is made visible.
function FinchShocker_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for FinchShocker_v2
...
hardware.plexon.chans = [2, 3, 4, 5, 6, 7]; %This is for the 6-ch Nanoclip
...
set(handles.ind_stimRecDirty, 'BackgroundColor', 'r', 'String', 'Dirty StimRecord')
set(handles.ind_plexonInit, 'BackgroundColor', 'r')
...
%Load schema image
axes(handles.axes_schema)
imshow('GUI Clip Image.bmp')

%Update stim plot
handles = updateStimPlot(handles);
%set(hObject, 'CloseRequestFcn', {@gui_close_callback, handles});

%Online/offline switch
if false
    % If the plexon library hasn't been installed, try to load it
    if ~exist('PS_InitAllStim', 'file')
        libdirs = {'/tdt/lib64', 'C:\opt\PlexonSDKs\MATLAB SDK for PlexStim 2.0 - 64 bit'};
...
function varargout = FinchShocker_v2_OutputFcn(hObject, eventdata, handles) 
...
cats = {'I1'; 'I2'; 'T1'; 'T2'; 'TI'; 'Reps'; 'Freq'}; %This order is necessary for the Plexon
params = [];
for i = 1:numel(cats)
...
function handles = fictiveLoop(handles)
%This is the main looping function for this program. Aims are to monitor
%the progress of the LabVIEW tCNS and A&N Recorder programs and log
%progress
global hardware
...
%Get the stimulation pattern you want to run from
if get(handles.check_stimInterleave, 'Value')
    p = get(handles.edit_stimSet, 'String');
    if strcmp(p, 'None')
        set(handles.text_message, 'String', 'Input channels to interleave')
    else
        pats = str2num(p);
    end
else
    pats = get(handles.popup_patSelect, 'Value');
    set(handles.text_curPat, 'String', num2str(pats));
end
...
%     dataIn = inputSingleScan(hardware.ni.session);
   ...
   %Look for a signal to indicate a change in system status
   if ~stimState && dataIn(2) %if stimState was low, but is now high... STATE CHANGE
       stimTrigger = true;
       stimState = true;
   end
   
   if stimTrigger && get(handles.check_stimInterleave, 'Value')
       %Randomly select which pattern to use
       ind = randi(numel(pats));
       patNum = pats(ind);
       
       %Update GUI Parameter display
       handles = updateGUIChan2(handles, handles.patFile.pattern(patNum));
       set(handles.text_curPat, 'String', num2str(patNum));
       
       %Update Plexon
       updatePlexonFromGUI(handles);
       
       %Pulse a digital output to register the change in pattern
       % 50% duty cycle, 10ms
       high = false(1,8); high(1) = true;
       low = false(1,8);
       for i = 1:patNum
            outputSingleScan(hardware.ni.session,high)
            pause(.01)
            outputSingleScan(hardware.ni.session,low)
            pause(.01)
       end
...
function handles = updateStimPlot(handles)
%Suck in the GUI parameters, plot the traces, and update the appropriate
%axes

%Set up constants
preTime = 100; %baseline pre-stim buffer in us (only for plotting)
postTime = 100;
tmin = inf; tmax = -inf;
imin = inf; imax = -inf;

%Update channels successively
 for i = handles.chans
   %Scrape and package parameters
   params = scrapeChan(handles, i);
   
   %Basic stim pattern
   c = ['handles.popup_ch' num2str(i) '_type'];
   source = eval(['get(' c ', ''Value'')']);
   if source == 1
       %Square pulse assembled from GUI params
       
       %Assemble timepoints
       intervals = [0, params(3), params(5), params(4)];
       timepnts = repelem(cumsum(intervals),2);
       
       %Assemble current magnitude series
       currents = [0, repelem([params(1), 0, params(2)], 2), 0];
       
   else
       %Other defined pulse...
       set(handles.text_message, 'String', 'Yeah, I dunno what to do here now')
       return
   end
   
   %Iterate based on number of repetitions/pulses specified
   switch params(6)
       case 0
           %Zero out it all...
           ts = [-preTime, timepnts, timepnts(end)+postTime];
           Is = zeros(size(ts));
       case 1
           %This is easy... just append the pre and post and do nothing
           ts = [-preTime, timepnts, timepnts(end)+postTime];
           Is = [0, currents, 0];
            
       otherwise
           %Calculate the interpulse interval fromt he repetition rate
           ipi = 1e6/params(7); %in us
           
           %Interpulse quiescent time
           qt = ipi - timepnts(end);
           
           inter = repmat([intervals, qt], 1, params(6));
           intervalsMult = repelem(cumsum(inter),2);
           
           cur = repmat([currents, 0, 0], 1, params(6));
           currentsMult = cur;
           
           ts = [-preTime, intervalsMult(1:end-2), intervalsMult(end-2)+postTime];
           Is = [0, currentsMult(1:end-2), 0];
           
   end
   
   %Keep track of the min/max for later plotting
   tmin = min([tmin, ts/1e3]); tmax = max([tmax, ts/1e3]);
   imin = min([imin, Is]); imax = max([imax, Is]);
   
   %Plot the resultant series
   c = ['handles.axes_ch' num2str(i)];
   eval(['axes(' c ')']);
   cla; %clear the axis
   line(ts/1e3, Is, 'LineWidth', .1); hold on
   line([-1000, 1000], [0,0], 'Color', 'k', 'LineStyle', ':', 'LineWidth', 1)
 end
 
 %Format the plots
 for i = handles.chans
     
     %Select the axes for format
     c = ['handles.axes_ch' num2str(i)];
     eval(['axes(' c ')']);
     
     if i == 3
         %Format the plot
         xlabel('Time (ms)'); ylabel('(\muA)')
     else
         %Format the plot
         xlabel(''); ylabel('')
     end
     
     %Calculate and set the figure limits
     ilim = round(1.2 * max(abs([imin, imax]))); ylim([-ilim, ilim])
     buffer = round(0.1 * (tmax - tmin)); xlim([tmin-buffer, tmax+buffer])
     set(gca, 'Box', 'off', 'TickDir', 'out', 'LineWidth', 1, 'FontSize', 8)
     
 end
 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plexon Configuration/Communication
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%Update status globals and GUI indicators
plexon_newly_initialised = true;
set(handles.ind_plexonInit, 'BackgroundColor', 'g')
...
if ~hardware.plexon.open || ~plexon_newly_initialised
    set(handles.text_message, 'String', 'Plexon doesn''t seem to be online... check that shit and try again.') 
    return
end
...
set(handles.ind_stimRecDirty, 'BackgroundColor', 'g', 'String', 'Clean StimRecord')
...
for i = handles.chans
   ...
   errParam1 = PS_SetRectParam (hardware.plexon.id, hardware.plexon.chans(i), params(1:5));
   
   %Set channel pattern repetitions
   errParam2 = PS_SetRepetitions (hardware.plexon.id, hardware.plexon.chans(i), params(6));
   
   %Set channel pattern rate
   errParam3 = PS_SetRate (hardware.plexon.id, hardware.plexon.chans(i), params(7));
   
   %Load channel settings
   errLoad = PS_LoadChannel(hardware.plexon.id, hardware.plexon.chans(i));
   
   if errParam1 || errParam2 || errParam3 || errLoad
       set(handles.text_message, 'String', ['Error updating Chan' num2str(i) ' state.'])
       return
   else
...
Param = [];
for i = handles.chans
    [Rate(i), errRat] = PS_GetRate (hardware.plexon.id, hardware.plexon.chans(i));
...
p1 = ['Chan 1: ' P{1}, ' ' Rep(1) ' ' Rate(1)];
p2 = ['Chan 2: ' P{2}, ' ' Rep(2) ' ' Rate(2)];
p3 = ['Chan 3: ' P{3}, ' ' Rep(3) ' ' Rate(3)];
p4 = ['Chan 4: ' P{4}, ' ' Rep(4) ' ' Rate(4)];
p5 = ['Chan 5: ' P{5}, ' ' Rep(5) ' ' Rate(5)];
p6 = ['Chan 6: ' P{6}, ' ' Rep(6) ' ' Rate(6)];
...
function push_loadPattern_Callback(hObject, eventdata, handles)
%Select and update stimulation pattern from file
scriptdir = fileparts(mfilename('fullpath'));
FileName = uigetfile([scriptdir, filesep, '*.m'], 'Select Pattern File to Load');
eval(['patterns = ' FileName(1:end-2) ';'])

%Copy the pattern out to the stable handle
if isempty(patterns)
    %Something went wrong...
    set(handles.text_messages, 'Pattern file not properly formatted... check it out and see whats up.')
    
else
    %Copy to the persistent structure
    handles.patFile.name = FileName(1:end-2);
    handles.patFile.pattern = patterns;
    
    %Update controls
    set(handles.popup_patSelect, 'String', 1:numel(handles.patFile.pattern), 'Value', 1)
    set(handles.text_patFile, 'String', FileName)
    
    if get(handles.check_usePatFile, 'Value')
        %Retrieve pattern selection
        patNum = get(handles.popup_patSelect, 'Value');
...
function popup_patSelect_Callback(hObject, eventdata, handles)
%Update if checkbox is marked
if get(handles.check_usePatFile, 'Value')
    %Retrieve pattern selection
    patNum = get(handles.popup_patSelect, 'Value');
...
function popup_patSelect_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function check_usePatFile_Callback(hObject, eventdata, handles)
%Update if checkbox is marked
if get(handles.check_usePatFile, 'Value')
    %Retrieve pattern selection
    patNum = get(handles.popup_patSelect, 'Value');
...
function handles = updateGUIChan2(handles, vals)
%This function updates the GUI controls for stim params for all channels.
%This is GUI only and has no effect on the hardware.
...
for i = 1:numel(cats)
    for j = handles.chans
        c =  ['handles.edit_ch' num2str(j) '_' cats{i} ', ''String'', num2str(vals.' cats{i} '(j))'];
        eval(['set(' c ');'])
    end
end

function edit_stimSet_Callback(hObject, eventdata, handles)

function edit_stimSet_CreateFcn(hObject, eventdata, handles)

if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function push_startFictiveLoop_Callback(hObject, eventdata, handles)
...
%Change button state
set(handles.push_startFictiveLoop, 'String', 'Looping!', 'BackgroundColor', 'g')
set(handles.push_stopFictiveLoop, 'String', 'Stop Looping!', 'BackgroundColor', 'r')
...
function push_stopFictiveLoop_Callback(hObject, eventdata, handles)
...
%Change button state
set(handles.push_sartFictiveLoop, 'String', 'Start Loop', 'BackgroundColor', [0.9, 0.9, 1])
set(handles.push_stopFictiveLoop, 'String', 'Stopped', 'BackgroundColor', [0.9, 0.9, 1])

% Update handles structure
guidata(hObject, handles);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NI Configuration/Communication
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%Test button... currently used to update the StimPlot from the user-entered
%parameters
...
function popup_ch1_type_Callback(hObject, eventdata, handles)

function popup_ch1_type_CreateFcn(hObject, eventdata, handles)

if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function check_stimInterleave_Callback(hObject, eventdata, handles)

function edit_ch1_Reps_Callback(hObject, eventdata, handles)

function edit_ch1_Reps_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch1_Freq_Callback(hObject, eventdata, handles)

function edit_ch1_Freq_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function popup_ch2_type_Callback(hObject, eventdata, handles)

function popup_ch2_type_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch2_Reps_Callback(hObject, eventdata, handles)

function edit_ch2_Reps_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch2_Freq_Callback(hObject, eventdata, handles)

function edit_ch2_Freq_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function popup_ch3_type_Callback(hObject, eventdata, handles)

function popup_ch3_type_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch3_Reps_Callback(hObject, eventdata, handles)

function edit_ch3_Reps_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch3_Freq_Callback(hObject, eventdata, handles)

function edit_ch3_Freq_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function popup_ch4_type_Callback(hObject, eventdata, handles)

function popup_ch4_type_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch4_Reps_Callback(hObject, eventdata, handles)

function edit_ch4_Reps_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch4_Freq_Callback(hObject, eventdata, handles)

function edit_ch4_Freq_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function popup_ch5_type_Callback(hObject, eventdata, handles)

function popup_ch5_type_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch5_Reps_Callback(hObject, eventdata, handles)

function edit_ch5_Reps_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch5_Freq_Callback(hObject, eventdata, handles)

function edit_ch5_Freq_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function popup_ch6_type_Callback(hObject, eventdata, handles)

function popup_ch6_type_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch6_Reps_Callback(hObject, eventdata, handles)

function edit_ch6_Reps_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function edit_ch6_Freq_Callback(hObject, eventdata, handles)

function edit_ch6_Freq_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function ind_plexonInit_Callback(hObject, eventdata, handles)

function ind_stimRecDirty_Callback(hObject, eventdata, handles)
